...
export default function BlogPage({ posts }: Props) {
  ...
  return (
    <>
      <Head>
        <title>Academic Writing Blog | Essays & Study Tips</title>
              ...
              <div className="flex flex-wrap justify-center gap-3">
                {categories.map((category) => (
                  <button key={category} onClick={() => setSelectedCategory(category)}
                    className={`px-4 py-2 rounded-full font-medium transition-all duration-200 ${selectedCategory === category ? 'bg-stone-900 text-white shadow-lg' : 'bg-white text-stone-600 hover:bg-stone-100 border border-stone-200'}`}>
                    {category}
                  </button>
                ))}
              </div>
                  ...
                  <div className="relative h-48 overflow-hidden">
                    ...
                    {post.type === 'transactional' && (
                      <div className="absolute top-4 right-4">
                        <span className="px-3 py-1 bg-amber-400 text-stone-900 rounded-full text-xs font-bold">Product Review</span>
                      </div>
                    )}
                  </div>
                  <div className="p-6">
                    ...
                    <h2 className="text-xl font-bold text-stone-900 mb-3 group-hover:text-stone-700 transition-colors duration-200 line-clamp-2">{post.title}</h2>
